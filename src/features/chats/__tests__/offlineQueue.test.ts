...
describe("offline queue state helpers", () => {
  test("creates queue entries from failed outbound messages", () => {
    const result = syncQueueFromThreads(
      [],
      [makeThread("msg-1")],
      new Set(),
      1000,
  ...
  test("auto-pauses entries once retry budget is exhausted", () => {
    const seed = syncQueueFromThreads(
      [],
      [makeThread("msg-3")],
      new Set(),
      1000,
    ...
    for (let attempt = 0; attempt < MAX_AUTO_RETRY_ATTEMPTS; attempt += 1) {
      queue = markQueueEntryAttemptFailed(
        queue,
        queue[0].id,
        "failed: timeout",
        1000 + attempt,
    ...
    expect(queue[0].lastError?.toLowerCase().includes("auto-paused")).toBe(
      true,
    );
  });
});
